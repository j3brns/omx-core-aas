...
export interface PlatformApiProps {
  ...
  readonly webhookDeliveryFn: lambda.IFunction;
...
export class PlatformApi extends Construct {
  ...
  constructor(scope: Construct, id: string, props: PlatformApiProps) {
    ...
    const webhookRegistryIntegration = new apigateway.LambdaIntegration(props.webhookRegistryFn, { proxy: true });
    const agentRegistryIntegration = new apigateway.LambdaIntegration(props.agentRegistryFn, { proxy: true });
    ...
    if (props.apiDomainName && props.apiCertificateArn) {
      const apiCustomDomain = new apigateway.DomainName(this, 'ApiCustomDomain', {
        ...
        certificate: acm.Certificate.fromCertificateArn(this, 'ApiCertificate', props.apiCertificateArn),
